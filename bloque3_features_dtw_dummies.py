## === BLOQUE 3 === FEATURE ENGINEERING + CLUSTERING DTW dummies ===
...
df_pred['es_precios_cuidados'] = df_pred['plan_precios_cuidados'].fillna(False).astype(int)
...
# === MERGE Y TRANSFORMACIÓN A DUMMIES ===
df_pred = df_pred.merge(df_clusters, on='product_id', how='left')
dummies = pd.get_dummies(df_pred['cluster_dtw'], prefix='cluster_dtw', drop_first=True)
df_pred = pd.concat([df_pred.drop(columns=['cluster_dtw']), dummies], axis=1)
